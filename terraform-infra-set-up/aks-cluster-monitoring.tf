...
# create managed grafana in primary region only
...
# data reader role for grafana to read metrics from prometheus
resource "azurerm_role_assignment" "grafana_datareaderrole" {
  scope                = azurerm_monitor_workspace.prometheus.id
  role_definition_name = "Monitoring Data Reader"
  principal_id         = azurerm_dashboard_grafana.grafana.identity.0.principal_id
}

# Monitoring Metrics Publisher role for grafana to publish metrics to prometheus
resource "azurerm_role_assignment" "grafana_metrics_publisher" {
  scope                = azurerm_monitor_workspace.prometheus.id
  role_definition_name = "Monitoring Metrics Publisher"
  principal_id         = azurerm_dashboard_grafana.grafana.identity.0.principal_id
}

# grant Grafana Admin role to current user 
...
### create data collection rule for each aks in different regions
### name format is must follow this: MSProm-<Location of Azure Monitor Workspace>-<Name of cluster resource>
resource "azurerm_monitor_data_collection_rule" "dcr" {
  for_each            = { for r in var.region_settings : r.name => r }
  description = "DCR for Azure Monitor Metrics Profile (Managed Prometheus)"

  name                        = "MSProm-${var.primary_region}-${azurerm_kubernetes_cluster.aks[each.value.name].name}"
  resource_group_name         = azurerm_resource_group.monitoring.name
  location                    = azurerm_resource_group.monitoring.location
  ...
  destinations {
    monitor_account {
      ...
      name               = "prometheus-${azurerm_monitor_workspace.prometheus.name}"
    }
  }

  data_flow {
    streams      = ["Microsoft-PrometheusMetrics"]  
    destinations = ["prometheus-${azurerm_monitor_workspace.prometheus.name}"]
  }
...
resource "azurerm_monitor_data_collection_rule_association" "dcra" {
  ...
  name                    = "MSProm-${var.primary_region}-aks-${each.value.short_name}"
  target_resource_id      = azurerm_kubernetes_cluster.aks[each.value.name].id
  data_collection_rule_id = azurerm_monitor_data_collection_rule.dcr[each.value.name].id
}


### create recording rules for each aks in different regions
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group" {
  for_each            = { for r in var.region_settings : r.name => r }

  name                = "NodeRecordingRulesRuleGroup-${azurerm_kubernetes_cluster.aks[each.value.name].name}"
  location            = azurerm_resource_group.monitoring.location
  resource_group_name = azurerm_resource_group.monitoring.name
  cluster_name        = azurerm_kubernetes_cluster.aks[each.value.name].name
  description         = "Node Recording Rules Rule Group"
  rule_group_enabled  = true
  interval            = "PT1M"
  scopes              = [azurerm_monitor_workspace.prometheus.id,azurerm_kubernetes_cluster.aks[each.value.name].id]
...
resource "azurerm_monitor_alert_prometheus_rule_group" "kubernetes_recording_rules_rule_group" {
  ...
  name                = "KubernetesRecordingRulesRuleGroup-${azurerm_kubernetes_cluster.aks[each.value.name].name}"
  location            = azurerm_resource_group.monitoring.location
  resource_group_name = azurerm_resource_group.monitoring.name
  cluster_name        = azurerm_kubernetes_cluster.aks[each.value.name].name
